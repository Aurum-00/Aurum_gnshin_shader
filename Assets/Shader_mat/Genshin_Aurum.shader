Shader "Unlit/Genshin_Aurum"
{
		...
		SubShader
	{
		...
		Varyings BaseColorVertex(Attributes input)
		{
		...
		half4 BaseColorFrag(Varyings input) : SV_Target
		{
			...
			float AOMask = saturate(LightMapColor.g * 2.0);

			float rampValue = input.HalfLambert * lerp(0.5, 1.0, AOMask) * (1.0 / _RampRange - 0.003);
			float rampVmove = 0;
			if (_Day > 0.5) {
			...
			half3 tightsRamp = step(abs(LightMapColor.a - 0.7), 0.05) * ShadowRamp2;
			half3 softCommonRamp = step(abs(LightMapColor.a - 0.5), 0.05) * ShadowRamp3;
			half3 hardSilkRamp = step(abs(LightMapColor.a - 0.3), 0.05) * ShadowRamp4;
			half3 metalRamp = step(abs(LightMapColor.a - 0), 0.05) * ShadowRamp5;
		...
		Pass
		{
			...
			#pragma vertex BaseColorVertex
			#pragma fragment BaseColorFrag
			ENDHLSL
		}
		Pass
		{
